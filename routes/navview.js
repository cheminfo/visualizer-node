...
exports  = module.exports = function(viewDirectory) {
    // Create the views directory
    fs.mkdirp(path.join(viewDirectory, 'test'));
    ...
    router.post('/save',
        ...
        function(req, res) {
            ...
            fs.writeFile(path.join(dir, name), content, function(err) {
                if(err) {
                    return res.status(400).json('Could not write file');
                }
                return res.status(200).json({});
            });
        }
    ...
    router.put('/move',
        ...
        function(req, res) {
            ...
            fs.rename(path.join(dir, name), path.join(newDir, newName), function(err) {
                if(err) return res.status(400).json('Unable to rename file');
            });
            return res.status(200).json({});
        }
    );
    ...
    function checkDir(name) {
        return function(req, res, next) {
            if(res.locals.parameters[name].indexOf('..') > -1) {
                ...
                return res.status(400).json('Directory does not exist');
            }
    ...
    function checkFile(file) {
        return function(req, res, next) {
            ...
            if(!f || f.indexOf('..') > -1 || f.indexOf('/') > -1) {
                ...
                return res.status(400).json('File did not pass security check');
            }
            next();
        }
    }
